...
func (p *Pinger) processPacket(recv *packet) error {
	...
	if m, err = icmp.ParseMessage(proto, recv.bytes); err != nil {
		fmt.Println("Error parsing the message")
...
func printBytes(decimal int) {
	// Convert decimal to byte slice
	byteSlice := make([]byte, 8) // Assuming 64-bit decimal
	binary.LittleEndian.PutUint32(byteSlice, uint32(decimal))
